    ...
    if process_this_frame:
        ...
        for face_encoding in face_encodings:
            ...
            if matches[best_match_index]:
                ...
                date_enter = datetime.datetime.now().date().strftime('%Y-%m-%d')
                heure_enter = datetime.datetime.now().time().strftime('%H:%M:%S')
                try:

                    status = 'present'

                    if heure_enter>datetime.time(8,0,0).strftime('%H:%M:%S') or heure_enter>datetime.time(14,0,0).strftime('%H:%M:%S'):
                        status = 'retard'

                    elif heure_enter>datetime.time(11,30,0).strftime('%H:%M:%S') or heure_enter>datetime.time(16,0,0).strftime('%H:%M:%S'):
                        status = 'abscent'
